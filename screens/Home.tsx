...
import { Book, Category, Profile } from "../types";
import ScreenWrapper from "@/components/ScreenWrapper";
import { getBooks } from "@/services/BookService";
import { getCategories } from "@/services/BookService";
import { bookCoverMap } from "@/utils/common";
...
const Home = ({ navigation, route }: { navigation: any; route: any }) => {
  const [myBooksData, setMyBooksData] = useState<Book[]>([]);
  ...
  useEffect(() => {
    ...
    setProfileData({ username, point: 250 });
    getCategories().then((categories) => setCategoriesData(categories));
    getBooks().then((books) => setMyBooksData(books));
  }, []);

  console.log("Profile: ", profileData);
  console.log("Categories: ", categoriesData);
  console.log("My Books: ", myBooksData);
  ...
  function renderMyBookSection(myBooks: Book[]) {
    const renderItem = ({ item, index }: { item: Book; index: number }) => {
      return (
        ...
        >
          {/* Book Cover */}
          <Image
            source={bookCoverMap[item.bookCover as keyof typeof bookCoverMap]}
            resizeMode="cover"
            style={{
              width: 180,
              height: 250,
    ...
    <ScreenWrapper>
      <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.black }}>
        ...
        <ScrollView style={{ marginTop: SIZES.radius }}>
          {/* Books Section */}
          <View>{renderMyBookSection(myBooksData)}</View>
